...
def create_pdf(type, data):
    ...
    if type == "receipt":
        ...
        # SMART IMAGE PLACEMENT (Fix for Requirement 4)
        if data.get('local_img_paths'):
            ...
            for p in data['local_img_paths']:
                if os.path.exists(p):
                    # Check space remaining on page (Page height ~297mm, margin ~15mm)
                    # Assuming image height ~80mm
                    current_y = pdf.get_y()
                    if current_y > 200: # If we are near bottom, new page
                        pdf.add_page()
                    
                    try: 
                        # Place image. w=0 means auto-scale width, h=80 limits height
                        pdf.image(p, x=15, w=100) 
                        pdf.ln(5) # small gap after image
                    except: pass
                    
        filename = f"DebitNote_{int(datetime.now().timestamp())}.pdf"

    else: # Statement Logic (Same as before)
        pdf.set_font("Helvetica", "", 12)
...
def inject_css(theme_name):
    ...
    <style>
        ...
        .small-btn {{
             display: inline-block; padding: 5px 10px; background-color: {t['primary']}; color: white; 
             border-radius: 5px; text-decoration: none; font-size: 0.8em; margin-top: 5px;
        }}
...
def main():
    ...
    if sel == "Dashboard":
        ...
        # FIX 1: Search is now a Dropdown populated from Database
        con_options = ["All Contractors"] + cons['Name'].tolist() if not cons.empty else ["All Contractors"]
        search_con = c1.selectbox("Filter by Contractor", con_options)
        ...
        if not df.empty:
            ...
            # Apply Filters
            if search_con != "All Contractors": 
                df = df[df['Contractor Name'] == search_con]
            if len(dr) == 2: 
                df = df[(df['Date'].dt.date >= dr[0]) & (df['Date'].dt.date <= dr[1])]
            df = df.sort_values(by="Date", ascending=(sort_order == "Date (Asc)"))
        card_end()

        # 2. Recent Activity Cards (FIX 2: Select/Print Recent Notes)
        ...
        if not df.empty:
            for i, row in df.head(5).iterrows():
                ...
                with rc1:
                    ...
                    st.caption(f"{row['Site Location']} • {row['Date'].date()}")
                with rc2:
                    st.markdown(f"**INR {row['Amount']}**")
                with rc3:
                    # Native Link Button to Open/Print PDF
                    if row['PDF Link'] and str(row['PDF Link']).startswith('http'):
                        st.link_button("📄 View/Print PDF", row['PDF Link'])
                    else:
                        st.caption("No PDF")
                card_end()
    ...
    # --- RAISE DEBIT NOTE ---
        ...
        # FIX 3: Persistence Keys (Solves Camera Refresh Issue)
        # We check session state for values. If not there, default is empty.
        ...
        with st.form("raise_form"):
            ...
            # We don't key 'con' because it resets on load anyway, but text inputs need keys
            dt = c2.date_input("Date")
            
            c3, c4 = st.columns(2)
            # Adding keys ensures if page reloads, text stays
            site = c3.text_input("Site Location", key="input_site") 
            amt = c4.number_input("Amount (INR)", min_value=0.0, key="input_amt")
            
            reason = st.text_area("Description / Reason", key="input_reason")
            ...
            if submitted:
                ...
                img_link_str = ",".join(links)
                curr_user = st.session_state['username']
                
                row_to_insert = [note_id, con, str(dt), amt, reason, site, img_link_str, pdf_link, curr_user]
                db_insert("DebitNotes", row_to_insert)
